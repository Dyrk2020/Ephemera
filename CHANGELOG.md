...
- feat: |Skill| 新增仓库内置只读 skill `cf-temp-mail-agent-mail`（`skills/cf-temp-mail-agent-mail/`），让 OpenClaw / Codex / Cursor 等 AI agent 凭用户提供的 Address JWT + API 地址读取邮箱、轮询验证码，绕开创建邮箱时的 Turnstile 人机验证；可通过 `npx degit upstream/cloudflare_temp_email/skills/cf-temp-mail-agent-mail` 安装
...
- feat: support `Oath2` 登录, 可以通过 `Github` `Authentik` 等第三方登录, 详情查看 [OAuth2 第三方登录](https://temp-mail-docs.example.com/zh/guide/feature/user-oauth2.html)
...
- fix: imap proxy server 不支持 密码 by @upstream (#356)
- worker 新增 `ANNOUNCEMENT` 配置, 用于配置公告信息 by @upstream (#357)
- fix: telegram bot 新建地址默认选择第一个域名 by @upstream (#358)
...
worker 配置文件新增 `DEFAULT_DOMAINS`, `USER_ROLES`, `USER_DEFAULT_ROLE`, 具体查看文档 [worker配置](https://temp-mail-docs.example.com/zh/guide/cli/worker.html#%E4%BF%AE%E6%94%B9-wrangler-toml-%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6)
...
- admin 可以删除 发件权限记录
- 添加中文邮件别名配置 `DOMAIN_LABELS` [文档](https://temp-mail-docs.example.com/zh/guide/cli/worker.html)
- 移除 `mail channels` 相关代码
- github actions 增加 `FRONTEND_BRANCH` 变量用于指定部署的分支 (#324)

## v0.5.1

- 添加 `mail-parser-wasm-worker` 用于 worker 解析邮件, [文档](https://temp-mail-docs.example.com/zh/guide/feature/mail_parser_wasm_worker.html)
...
- 增加使用 `resend` 发送邮件, `resend` 提供 http 和 smtp api, 使用更加方便, 文档: https://temp-mail-docs.example.com/zh/guide/config-send-mail.html
...
- 增加  telegram bot 文档 https://temp-mail-docs.example.com/zh/guide/feature/telegram.html
...
- feat: requset_send_mail_access default 1 balance by @upstream in https://github.com/upstream/cloudflare_temp_email/pull/143
