...
| 变量名                     | 类型        | 说明                                       | 示例                                 |
| -------------------------- | ----------- | ------------------------------------------ | ------------------------------------ |
| `DOMAINS`                  | JSON        | 用于临时邮箱的所有域名, 支持多个域名       | `["example.com", "example.org"]` |
...
| `DEFAULT_DOMAINS`                     | JSON      | 默认用户可用的域名(未登录或未分配角色的用户)                                                                                      | `["example.com", "example.org"]`      |
...
| `DOMAIN_LABELS`                       | JSON      | 对于中文域名，可以使用 DOMAIN_LABELS 显示域名的中文展示名称                                                                       | `["中文.example.com", "example.org"]` |
...
> 文档、[#1035](https://github.com/upstream/cloudflare_temp_email/issues/1035) 与
...
>
> 通过用户界面部署时 `USER_ROLES` 请配置为此格式 `[{"domains":["example.com","example.org"],"role":"vip","prefix":"vip"},{"domains":["example.com","example.org"],"role":"admin","prefix":""}]`
>
> CLI 部署时 `USER_ROLES` 请参考 `worker/wrangler.toml.template` 配置为此格式 `[{ domains = ["example.com", "example.org"], role = "vip", prefix = "vip" }, { domains = ["example.com", "example.org"], role = "admin", prefix = "" }]`
