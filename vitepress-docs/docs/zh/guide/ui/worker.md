...
4. 下载 [worker.js](https://github.com/upstream/cloudflare_temp_email/releases/latest/download/worker.js)
...
5. 回到 `Overview`，找到刚刚创建的 worker，点击 `Edit Code`, 删除原来的文件，上传 `worker.js`, 点击 `Deploy`
    ...
    >
    > 参考: [issues156](https://github.com/upstream/cloudflare_temp_email/issues/156#issuecomment-2079453822)
...
6. 点击 `Settings` -> `Variables and Secrets`, 如图所示添加变量
    ...
    >
    > 对于 `USER_ROLES` 请配置为此格式 `[{"domains":["example.com","example.org"],"role":"vip","prefix":"vip"},{"domains":["example.com","example.org"],"role":"admin","prefix":""}]`
