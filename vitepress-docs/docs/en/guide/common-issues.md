...
| `Subdomain cannot send emails`                                     | [Reference](https://github.com/upstream/cloudflare_temp_email/issues/515) |
...
| `SMTP_CONFIG` configured but sending fails | Make sure the JSON key is **your own sending domain** (e.g. `your-domain.com`), not the example `example.com`. See [Configure Email Sending](/en/guide/config-send-mail#send-emails-using-smtp) |
