...
| -------------------------- | ----------- | ---------------------------------------------------------------------- | ------------------------------------ |
| `DOMAINS`                  | JSON        | All domains for temporary email, supports multiple domains             | `["example.com", "example.org"]` |
...
| `DEFAULT_DOMAINS`                     | JSON      | Default domains available to users (not logged in or users without assigned roles)                                                                                                                                | `["example.com", "example.org"]`      |
...
| `DOMAIN_LABELS`                       | JSON      | For Chinese domains, you can use DOMAIN_LABELS to display Chinese names                                                                                                                                           | `["中文.example.com", "example.org"]` |
...
> docs,
> [#1035](https://github.com/upstream/cloudflare_temp_email/issues/1035) and
...
>
> When deploying through UI, configure `USER_ROLES` in this format: `[{"domains":["example.com","example.org"],"role":"vip","prefix":"vip"},{"domains":["example.com","example.org"],"role":"admin","prefix":""}]`
>
> When deploying via CLI, refer to `worker/wrangler.toml.template` and configure `USER_ROLES` in this format: `[{ domains = ["example.com", "example.org"], role = "vip", prefix = "vip" }, { domains = ["example.com", "example.org"], role = "admin", prefix = "" }]`
