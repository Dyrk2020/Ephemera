...
3. Enter the deployed worker address. It must be the backend API root URL, start with `https://`, and must not include a trailing `/`. Click generate, and if successful, a download button will appear. You will get a zip package.
    - The worker domain here is the backend API domain. For example, if I deployed at `https://temp-email-api.example.com`, then fill in `https://temp-email-api.example.com`
    ...
    > [!NOTE]
    > You can also deploy manually. Download the zip from here: [frontend.zip](https://github.com/upstream/cloudflare_temp_email/releases/latest/download/frontend.zip)
    >
